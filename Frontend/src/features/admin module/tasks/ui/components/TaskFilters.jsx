...
import { Search, Filter, UserCheck, AlertCircle, X } from "lucide-react";
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="relative">
          <select
            ...
            className="w-full bg-[var(--input)] border border-[var(--input-border)] text-[var(--text-primary)] rounded-lg px-3 py-2 text-xs md:text-sm focus:outline-none focus:border-[var(--primary)] transition-colors cursor-pointer appearance-none"
          >
            <option value="all">Assignee: All Team Members</option>
            {employees.map((emp) => (
          ...
          <UserCheck className="w-3.5 h-3.5 text-[var(--text-muted)] absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" />
        </div>
      </div>
